...
for j = 1:n
    ...
    normx = norma(R(j:end,j)); % Norm of large vector
%     s = -sign(R(j,j));        % +/- 1 scalar
    if R(j,j) < 0
        s = 1.0;
    else
        s = -1.0;
    end
    u1 = R(j,j) - s*normx;    % Scalar
%     w = R(j:end,j)/u1;
    w = zeros(n-j+1,1);       % Always gets smaller (sx1) s = i-j+1 (also in C)
    for i = j+1:n
        w(i-j+1) = R(i,j)/u1; % In C (i-j) only
    end
    w(1) = 1;                 % Simple assignment
    tau = -s*u1/normx;        % Scalar
    % -- R := HR, Q := QH
    Rhelp = w'*R(j:end,:);    % Size (1xs)(sxn) 
    Qhelp = Q(:,j:end)*w;     % Size (nxs)(sx1)
    R(j:end,:) = R(j:end,:)-(tau*w)*Rhelp;  %(sxn) = (sxn) - (sx1)*(1xn)
    Q(:,j:end) = Q(:,j:end)-Qhelp*(tau*w)'; %(nxs) = (nxs) - (nx1)*(1xs)
end
